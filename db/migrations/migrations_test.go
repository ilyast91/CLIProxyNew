...
func TestInitialMigrationCoversDesignedSchema(t *testing.T) {
	...
	assertContains(t, up, "function manage_usage_event_partitions")
	assertContains(t, up, "select manage_usage_event_partitions()")
	assertContains(t, down, "drop function if exists manage_usage_event_partitions")
