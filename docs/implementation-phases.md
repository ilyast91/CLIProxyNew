...
- [ ] R9.A.1 OAuth flow: `internal/auth/oauth` (FlowManager) — callback-flow (Codex/Claude/Antigravity) + device-flow (Kimi/xAI), сессии в oauth_sessions, `Store.Save` после exchange (Postgres lifecycle, documented admin status/cancel и tests готовы; provider adapters pending)
...
- [x] R9.A.6 allow-list моделей + model-mapping (через model_overrides; admin read/upsert/delete с audit и OpenAPI)
