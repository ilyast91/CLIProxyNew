...
graph TB
    ...
    subgraph CLIProxyNew["CLIProxyNew — бизнес-слой"]
        ...
        OAUTHIMP["httpapi<br/>OAuth JSON import/export R9.A.7"]
...
## 6. Поток: management — импорт OAuth credential (R9.A.7)

Текущий v1 не запускает интерактивный callback/device OAuth login. Администратор
получает полный credential JSON вне приложения и передаёт его в
`POST /api/v1/admin/oauth/import` как `application/json` body или как файл в
multipart-поле `file`. Для обоих форматов действует общий лимит credential
1 MiB и единый validation/register/audit pipeline.

```mermaid
sequenceDiagram
    participant A as Админ
    participant H as httpapi mgmt
    participant M as coreauth.Manager
    participant Store as encrypted Store
    participant DB as Postgres

    A->>H: POST admin oauth import JSON body или multipart file
    H->>H: media type + лимит 1 MiB + decode + validate
    H->>M: проверить duplicate provider + email
    H->>M: Register credential без внешнего ID
    M->>Store: Save credential
    Store->>DB: UPSERT upstream account AES-GCM + audit
    M-->>H: credential с SDK-managed ID
    H-->>A: 201 id provider email
```

Экспорт выполняется через
`GET /api/v1/admin/oauth/accounts/{accountID}/export`: после проверки OAuth-типа
и записи audit handler отдаёт полный JSON как attachment. JSON содержит секреты
и не логируется. Возможный интерактивный login-flow описан только как
[post-v1 дизайн](design/r9-oauth-and-testing.md).
...
### `internal/auth/oauth` — зарезервировано для post-v1 OAuth login (R9.A.1)
Пакет не участвует в текущем runtime и не содержит provider-specific flow.
Интерактивный callback/device login отложен до отдельной итерации. Его нельзя
реализовывать импортом upstream `internal/*`; будущий вариант должен использовать
reviewable публичные SDK-контракты и сохранять multi-replica свойства. Текущий
v1 работает через JSON import/export в `internal/httpapi` (R9.A.7). См.
[отложенный дизайн](design/r9-oauth-and-testing.md).
...
- **Management-API** (`/api/v1/*`) — через `api.WithRouterConfigurator`:
  ...
  - `/api/v1/admin/users`, `/api/v1/admin/keys`, `/api/v1/admin/providers/keys`,
    `/api/v1/admin/quota/*`, `/api/v1/admin/accounts/test`,
    `/api/v1/admin/models`, OAuth session lifecycle и
    `/api/v1/admin/oauth/accounts/{accountID}/export|/api/v1/admin/oauth/import`
    (R9.A). Interactive OAuth `start`/`complete` endpoints в v1 отсутствуют.
- **Системные роуты:** отдельный SDK configurator регистрирует `/healthz`
  ...
  сгенерированный из `openapi.yaml`; CI проверяет отсутствие drift. `/docs/`
  обслуживается `swaggest/swgui/v5emb`: Swagger UI 5.32.8 и все JS/CSS/favicon
  встроены в Go-бинарник и читают локальный `/openapi.json`. `/docs` и
  `/docs/` отдают один UI handler. `/metrics` отдаёт Prometheus registry.
...
- **Метрики (R6):** `internal/metrics` держит private Prometheus registry;
  ...
  upstream-hook, usage queue depth, verified API-key auth cache (`hit|miss`) и pgx
...
- `api_key_auth` (TTL 10с): SHA-256(API-key) + identity source → principal
  после успешного bcrypt и проверки `users.status`; plaintext не хранится,
  local revoke/admin block инвалидируют entries, остальные реплики сходятся
  не позднее TTL по R2.4;
...
### Внешние runtime-ресурсы (R6.6)

Бинарник не загружает CDN assets, remote fonts/scripts/styles или скрытые SaaS
dependencies. Разрешены только явно настроенные Postgres, LDAP, telemetry,
system proxy и upstream/provider endpoints, составляющие функцию сервиса.
`scripts/security-audit.sh` проверяет runtime/build source и запрещает CDN hosts,
remote browser assets, CDN-варианты `swaggest/swgui/v*cdn` и build tag
`swguicdn`.
