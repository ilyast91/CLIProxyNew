...
- ✅ **Решено:** роль администратора определяется по LDAP **через конфиг**
  сервиса (имя группы и/или атрибут + значение задаются в `config.yaml`),
  что даёт гибкость под конкретную схему каталога без правок кода.
- ❓ **Открыто:** точный формат конфиг-секции LDAP (DN группы для админов,
  атрибут/значение, base DN, bind DN, filter шаблон) — зафиксировать в
  R1-дизайне.
