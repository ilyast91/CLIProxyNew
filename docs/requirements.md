...
- Management-поверхность (R9): пользовательские операции (свои API-keys,
  ...
  управление пользователями, тестирование аккаунтов, список моделей).
...
- **Квоты и rate-limit** — на первой версии не реализуются (отложены).
...
- **R9.A.1** Настройка **OAuth для провайдеров с подписками** (Codex/Claude/
  ...
  device-flow/authorization-flow через ядро (`sdkAuth.Manager.Login`),
  хранение полученных upstream-credentials в БД (зашифрованные, R5).
...
- **R9.A.7** **Экспорт/импорт OAuth-credentials в JSON.** Администратор может
  ...
  - ✅ **Решено:** **содержимое JSON — полные credentials** (access_token +
    ...
    секретом (транспорт только по HTTPS, не логировать,短期 TTL доступа).
