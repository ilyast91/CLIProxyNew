...
- ❓ **Открыто:** TTL session, политика refresh, max session lifetime.
- ❓ **Открыто:** SSO-flow — отдельный login endpoint + cookie, или API-only.
- ❓ **Открыто:** синхронизация LDAP-групп в роли сервиса или live-lookup.
...
| ADR-3 | Глубина LDAP-интеграции (прямой bind vs через IdP) | ❓ |
